...
seglen = [fixStimulus.stimTime fixStimulus.interTime fixStimulus.stimTime fixStimulus.interTime fixStimulus.responseTime...
    fixStimulus.trialTime-(2*fixStimulus.interTime + 2*fixStimulus.stimTime + fixStimulus.responseTime) - 0.5];% same as task{1}.seglen
...
if any(task.thistrial.thisseg == stimulus.stimulusSegments)
    ...
    if task.thistrial.thisseg == stimulus.stimulusSegments(1) 
        newOri = ceil(rand(1)*numOrientations);
        while stimulus.oriNum == newOri
            newOri = ceil(rand(1)*numOrientations);
        end
        stimulus.oriNum = newOri;
    elseif task.thistrial.thisseg == stimulus.stimulusSegments(2)
        newOri = stimulus.oriNum + numOrientations/2;
        newOri = 1+mod(newOri-1,numOrientations);
        stimulus.oriNum = newOri;
    end
...
stimulus.stimulusSegments = 2:2:(length(task{2}{1}.seglen) - 1);
...
in = 0;%stimulus.inner/stimulus.width;
...
if ~isempty(whichInterval)
    ...
    % write out what the strength is12122
...
elseif task.thistrial.thisseg == 5%6
    ...
    % if this is the inter trial interval
elseif task.thistrial.thisseg == 6
    ...
    fixStimulus.thisColor = myscreen.background;%fixation disappears between stimuli
    myscreen = writeTrace(0,task.fixStairTrace,myscreen);
end
